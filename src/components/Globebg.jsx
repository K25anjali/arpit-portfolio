...
const fullFragmentShader = `#version 300 es
precision highp float;
out vec4 O;
uniform float time;
...
vec3 march(in vec3 p, vec3 rd) {
  float d=.2, t=.0, c=.0, k=mix(.9,1.,rnd(rd)), maxd=length(p)-1.;
  vec3 col=vec3(0);
...
function Globebg() {
  ...
  const [shaderSource, setShaderSource] = useState(fullFragmentShader);
  ...
  useEffect(() => {
    if (!gl) return;
    try {
      const program = initShaderProgram(gl, vertexShaderSrc, shaderSource);
      ...
      const render = (time) => {
        ...
        gl.uniform2f(resUniformLocation, canvasRef.current.width, canvasRef.current.height);
        gl.uniform2f(moveUniformLocation, 0.0, 0.0); // No interaction yet
